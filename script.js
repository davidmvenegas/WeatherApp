...
function showWeatherData(data) {
    ...
    data.daily.forEach((day, idx) => {
        if(idx == 0){
            ...
            `
            <img src="http://openweathermap.org/img/wn/${day.weather[0].icon}@4x.png" alt="weather icon" class="w-icon-main">
            <div class="other">
            <div class="current-day">${window.moment(day.dt * 1000).format('ddd')}</div>
            <div class="current-temp">Night - ${day.temp.night}°F</div>
            <div class="current-temp">Day - ${day.temp.day}°F</div>
            </div>
            `
        } else {
            otherDayForcast += 
